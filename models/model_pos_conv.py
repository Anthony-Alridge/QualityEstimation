...
from models.neuralnet import Rishinet5
import pickle
  ...
  for l in lines:
      ...
      if vec is not None:
        ...
        for tag in list(tagdict.keys()):
            counts.append(float(vec.count(tag)))
        sentences_vectors.append(counts)
      else:
        print("didn't work :", l)
...
def get_labels_and_data(type='train'):
    ...
    lines_en_nltk = get_embeddings('../data/en-de/' + type + '.ende.src',nlp_en,'en')

    f = open('../data/en-de/' + type + '.ende.mt', encoding='utf-8') # Open file on read mode
    lines_de = f.read().split("\n") # Create a list containing all lines
    f.close() # Close file

    lines_de_nltk = get_embeddings('../data/en-de/' + type + '.ende.mt',nlp_de,'de')
    if type != 'test':
    ...
    datas = []
    for i,j,k,l in zip(lines_en, lines_de, lines_en_nltk, lines_de_nltk):
      if i is '':break
      datas.append([tokeniser.encode(i,max_length=90,pad_to_max_length=True),tokeniser.encode(j,max_length=90,pad_to_max_length=True), k+l])
